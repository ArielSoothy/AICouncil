...
export async function fetchSharedTradingData(symbol: string): Promise<SharedTradingData> {
  console.log(`🔍 Fetching shared market data for ${symbol.toUpperCase()}...`);
  ...
  try {
    ...
    console.log(
      `✅ Market data fetched: $${data.quote.price.toFixed(2)}, RSI ${data.technical.rsi.toFixed(
        2
      )}, ${data.news.length} news articles`
    );
