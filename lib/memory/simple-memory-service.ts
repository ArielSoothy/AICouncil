...
export class SimpleMemoryService {
  ...
  async storeEpisodicMemory(memory: Omit<EpisodicMemory, 'id' | 'created_at' | 'updated_at' | 'type'>): Promise<EpisodicMemory | null> {
    try {
      ...
      console.log('✅ Stored episodic memory:', episodicMemory.query.substring(0, 50) + '...')
  ...
  async storeSemanticMemory(memory: Omit<SemanticMemory, 'id' | 'created_at' | 'updated_at' | 'type'>): Promise<SemanticMemory | null> {
    try {
      ...
      if (existing) {
        ...
        console.log('✅ Updated semantic memory:', existing.fact.substring(0, 50) + '...')
        return existing
      }
      ...
      console.log('✅ Stored semantic memory:', semanticMemory.fact.substring(0, 50) + '...')
  ...
  async storeProceduralMemory(memory: Omit<ProceduralMemory, 'id' | 'created_at' | 'updated_at' | 'type'>): Promise<ProceduralMemory | null> {
    try {
      ...
      console.log('✅ Stored procedural memory:', proceduralMemory.rule_name)
  ...
  async getMemoryStats(): Promise<MemoryStats> {
    ...
    console.log('📊 Generated memory stats:', stats)
    return stats
  }
  ...
  async searchEpisodicMemories(query: string, limit: number = 5): Promise<EpisodicMemory[]> {
    ...
    console.log(`🔍 Found ${matches.length} similar episodic memories for query: "${query.substring(0, 50)}..."`)
    return matches
  }
